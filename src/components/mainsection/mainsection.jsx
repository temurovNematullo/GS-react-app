...
import {
  fetchMainProducts,
  setMainProductId,
} from "../../redux/Slices/mainPageProductSlice";
import { themeContext } from "../../providers/theme";

export default function Mainsection() {
  const { theme } = useContext(themeContext);

  const { mainProducts, id } = useSelector(
    (state) => state.mainPageProductReducer
  );
  ...
  const prevLock = () => {
    dispatch(
      setMainProductId((id - 1 + mainProducts.length) % mainProducts.length)
    );
  };
  ...
  const prevIMG = () => {
    if (currentLock?.imageUrl?.length > 0) {
      setCurrentindexIMG(
        (currentindexIMG - 1 + currentLock.imageUrl.length) %
          currentLock.imageUrl.length
      );
    }
  };
